...
public abstract class TowerSpawnerLogic {
        ...
        if (!this.isPlayerInRange()) {
            ...
            if (world.isClient) {
                ...
                while(true) {
                    ...
                    if (world.doesNotCollide(((EntityType)optional.get()).createSimpleBoundingBox(g, h, k)) && SpawnRestriction.canSpawn((EntityType)optional.get(), world.getWorld(), SpawnReason.SPAWNER, new BlockPos(g, h, k), world.getRandom())) {
                        label97: {
    ...
    public Entity getRenderedEntity() {
        if (this.renderedEntity == null) {
            ...
            if (this.spawnEntry.getEntityTag().getSize() == 1 && this.spawnEntry.getEntityTag().contains("id", 8) && this.renderedEntity instanceof MobEntity) {
                ((MobEntity)this.renderedEntity).initialize(this.getWorld(), this.getWorld().getLocalDifficulty(this.renderedEntity.getBlockPos()), SpawnReason.SPAWNER, (EntityData)null, (CompoundTag)null);
            }
        }
